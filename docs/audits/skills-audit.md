...
| `aura_reading` | FIX | Passive emotional read — requires enemy AI to *have* moods; deterministic v1 AI can expose a stance enum (aggressive/defensive/fleeing/desperate) — map "emotion" to AI stance and it's engine-honest. `is_passive:1` yet has range/target and an active read pattern — flag semantics (always-on aura within range fits passive; keep). L6 "reveals lying" = dialogue layer (KAN-6+). | Reword effect: "reveals the target's current AI stance"; tag `needs: ai-stance-exposure`. **Substrate SHIPPED wave 3a (2026-08-11):** every AI combatant carries a serialized `ai_stance` (aggressive/hunting/defensive/building — table in `simulation/enemy_ai.gd`), exposed on `view_combatants`; the SKILL itself still rides the content pass. |
