#!/usr/bin/env bash
# Render the demo-slice VERDICT card (ui/screens/verdict_card.tscn) to a PNG for
...
# Output: $VERDICT_OUT if set, else <project>/verdict_render.png.
...
if [ -z "$GODOT" ]; then
	echo "SKIP: no Godot 4 binary — verdict card not rendered (this is not a pass)"
	echo "      install Godot 4.7 and expose it as 'godot' on PATH or via \$GODOT_BIN"
	exit 3
fi

PROJECT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
OUT="${VERDICT_OUT:-$PROJECT_DIR/verdict_render.png}"
...
VERDICT_OUT="$OUT" xvfb-run -s "$XVFB_SCREEN" -a "$GODOT" --path "$PROJECT_DIR" -s scripts/verdict_preview.gd
STATUS=$?
echo "render -> $OUT"
exit $STATUS
