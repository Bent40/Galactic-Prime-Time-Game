...
func test_dal_counts_match_validator() -> void:
	...
	assert_eq(dal.skills().size(), 48, "skills (batch B imports the 5 passover rows, ids 46-50)")
	assert_eq(dal.skill_thresholds().size(), 88, "thresholds (batch B adds L5/L6 rows for ids 46-50)")
