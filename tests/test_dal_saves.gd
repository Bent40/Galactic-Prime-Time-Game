...
func test_dal_counts_match_validator() -> void:
	...
	assert_eq(dal.skills().size(), 58, "skills (tier-2 wave 1 imports the 10 BLESSED ladders, ids 51-60)")
	assert_eq(dal.skill_thresholds().size(), 98, "thresholds (tier-2 wave 1 adds the L5 mastery rungs, ids 93-102)")
